<x-app-layout>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <form action="{{ route('places.index') }}" method="GET" class="flex">
                        ...
                        <tr class="intro-x">
                            <td>
                                <a href="{{ route('places.show', $place->id) }}"
                                    class="font-medium whitespace-nowrap">{{ $place->name }}</a>
                            </td>
                            <td class="text-center">{{ $place->city }}</td>
                            <td class="text-center">{{ $place->state }}</td>
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <x-forms.buttons.edit :route="route('places.edit', $place->id)" />
                                    <x-forms.buttons.delete :route="route('places.destroy', $place->id)" />
                                </div>
                            </td>
                        </tr>
                    @endforeach
        ...
        {{ $places->links('layouts.paginator') }}
